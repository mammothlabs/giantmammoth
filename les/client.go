...
func New(stack *node.Node, config *ethconfig.Config) (*LightEthereum, error) {
	...
	chainConfig, genesisHash, genesisErr := core.SetupGenesisBlockWithOverride(chainDb, config.Genesis, config.OverrideBerlin, config.OverrideShanghai)
	...
	leth := &LightEthereum{
		...
		engine:         ethconfig.CreateConsensusEngine(stack, chainConfig, nil, false, chainDb, nil, genesisHash),
